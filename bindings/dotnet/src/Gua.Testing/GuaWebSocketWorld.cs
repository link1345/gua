...
public sealed partial class GuaWebSocketContext
{
    public string GetWorldObjectTreeJson(GuaObservationProfile profile = GuaObservationProfile.Debug) =>
        Raw(new { type = "get_world_object_tree" });
    ...
    private static object WorldQueryCommand(GuaWorldSelector selector)
    {
        ...
        if (selector.VisibleToPlayer is not null) command["visibleToPlayer"] = Filter(selector.VisibleToPlayer);
        if (selector.Active is not null) command["active"] = Filter(selector.Active);
        if (selector.State is { } state) {
            command["stateKey"] = state.Key;
            command["stateType"] = StateType(state.Value) ?? throw new ArgumentException($"World state '{state.Key}' must be a primitive JSON value.", nameof(selector));
    ...
    private static int? StateType(object? value) => value switch { null => 0, string => 1, byte or sbyte or short or ushort or int or uint or long or ulong or float or double or decimal => 2, bool => 3, _ => null };
