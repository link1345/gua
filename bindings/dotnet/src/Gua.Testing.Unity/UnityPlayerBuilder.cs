...
public static class UnityPlayerBuilder
{
    public static string Build(string scenePath, UnityPlayerBuildOptions? options = null)
    {
        ...
        var platform = UnityPaths.ResolvePlatform(options.Platform);
        var output = Path.GetFullPath(options.OutputPath ?? UnityPaths.DefaultPlayerOutput(platform));
        Directory.CreateDirectory(Path.GetDirectoryName(output)!);
        var log = output + ".build.log";
        var args = new List<string> { "-batchmode", "-quit", "-projectPath", project, "-executeMethod", "Gua.Unity.Editor.GuaUnityEditorCommands.BuildPlayer", "-guaScene", scene, "-guaOutput", output, "-guaPlatform", UnityPaths.PlatformArgument(platform), "-logFile", log };
        ...
        if (process.ExitCode != 0 || (!File.Exists(output) && !Directory.Exists(output))) throw new InvalidOperationException($"Unity player build failed with exit code {process.ExitCode}. editor='{unity}', project='{project}', scene='{scene}', platform='{platform}', output='{output}', log='{log}'.\n{Read(log)}");
        return output;
    }
